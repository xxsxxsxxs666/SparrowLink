...
### data conversion
#python3 /public/home/v-xiongxx/Graduate_project/nnUnetv2/nnUNet/nnunetv2/dataset_conversion/Dataset800_CCTA1.py \
...
do
  ...
  do
    ...
    python3 SparrowLink_Post_Process_v4.py \
    --stage=1 \
    ...
    --save_dir=${experiments_path}/first_stage/${model}/${phase}_${mode}_infer_detection \
...
#    --CS_M=${experiments_path}/first_stage/${model}/${phase}_${mode}_infer \
#    --CS_A=${experiments_path}/first_stage/${model}/registration/${auxiliary_phase}_${mode}_infer_SyNRA_reg_1 \
#    --save_dir=${experiments_path}/first_stage/${model}/${phase}_${mode}_infer_detection_change_connectivity \
#    --GT=${data_path}/${mode}/${phase}/label_crop_clip \
#    --multiprocess \
#    --max_broken_size_percentage=0.05
#  done
#done
...
do
  for phase in "main" "auxiliary" ;
  do
    for postfix in "_sphere" "_sphere_GT" "_sphere_com";
    do
...
python3 second_stage_main_v4.py \
--mode=train \
...
--select_file=${experiments_path}/first_stage/${model}/main_train_infer_detection/process_record.json \
...
--CS_DLC=${experiments_path}/first_stage/${model}/main_train_infer_sphere_com \
...
--CS_W=${results}/${task_name}/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_0/checkpoint_final.pth \
--workers=8|| { echo 'Error during training of refined segmentation.'; exit 1; }
fi
## 2. inference
## use test mode
...
do
  ...
  --CS_DLC=${experiments_path}/first_stage/${model}/main_${mode}_infer_sphere_com \
  --I_A=${experiments_path}/first_stage/${model}/registration/${auxiliary_phase}_${mode}_img_SyNRA_reg_1 \
  --CS_A=${experiments_path}/first_stage/${model}/registration/${auxiliary_phase}_${mode}_infer_SyNRA_reg_1 \
  --pretrain_weight_path=${experiments_path}/second_stage/${second_stage_dir}/checkpoint/best_metric_model.pth \
  --workers=8 || { echo 'Error during inferring of refined segmentation.'; exit 1; }
done
...
python3 post_processing/select_region.py \
--data_path=${experiments_path}/first_stage/${second_stage_dir}/main_test_infer_detection \
--save_path=${experiments_path}/second_stage/${second_stage_dir}/main_test_infer_detection_1 \
--data_postfix="_csaDM" \
--save_postfix="_csaDM_TWO" \
--hierarchical=/* \
--separate_folder \
--num=2 || { echo 'Error during moving file.'; exit 1; }
...
for postfix in "CS_M" "_RCS" "_ARCS" "_ARCS_TWO" "_CS_M_TWO" "_RCS_SELECTED" "_RCS_SELECTED_TWO" "_ARCS_SELECTED" "_ARCS_SELECTED_TWO" "_RCS_NEW" "_RCS_NEW_TWO" "_ARCS_NEW" "_ARCS_NEW_TWO" "_csaDM" "_csaDM_TWO";
do
  python3 caculate_metric.py \
...
for postfix in "CS_M" "_RCS" "_ARCS" "_RCS_SELECTED" "_ARCS_SELECTED" "_RCS_NEW" "_ARCS_NEW" "_csaDM" "_csaDM_TWO";
do
  python3 SparrowLink_metric.py \
  --seg=${experiments_path}/second_stage/${second_stage_dir}/main_test_infer_detection_1 \
  --seg_find=*/*${postfix}.nii.gz \
